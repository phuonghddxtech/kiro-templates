...
| **Mobile (JS/TS team)** | React Native (Expo) | Bare RN | Ionic, Cordova |
| **Mobile (Dart team)** | Flutter 3+ | — | Xamarin |
| **Backend Framework (PHP)** | Laravel 11+ | — | CodeIgniter, Symfony (unless needed) |
| **API Style** | REST (default) | tRPC (fullstack TS) | GraphQL (unless needed) |
| **Language** | TypeScript (always) | PHP 8.2+ (Laravel) | Plain JavaScript |
| **Database** | PostgreSQL | MySQL (Laravel projects) | — |
| **ORM** | Prisma (Node) | Eloquent (Laravel) | Sequelize, TypeORM |
...
## 🐘 Laravel Stack (PHP Projects)

### When to use Laravel vs Node/Express

| Tiêu chí | Laravel (PHP) | Express (Node) |
|----------|--------------|----------------|
| **Team** | PHP team | JS/TS team |
| **Rapid API dev** | ✅ Batteries included | Requires more setup |
| **Admin panels** | ✅ Filament, Nova | Custom build |
| **Full-stack PHP** | ✅ Blade + Livewire | — |
| **Real-time** | Reverb / Pusher | ✅ Socket.io native |
| **Microservices** | Less common | ✅ Preferred |
...
### Laravel Approved Packages
| Purpose | Package |
|---------|---------|
| **Auth (API/SPA)** | Laravel Sanctum |
| **Auth (OAuth2)** | Laravel Passport |
| **Queue** | Laravel Queue (Redis driver) |
| **Cache** | Laravel Cache (Redis driver) |
| **Search** | Laravel Scout + Meilisearch |
| **File Storage** | Laravel Filesystem (S3) |
| **Admin Panel** | Filament |
| **API Docs** | Scribe or L5-Swagger |
| **Testing** | Pest PHP |
| **Logging** | Laravel Log (Monolog) |

### Laravel Key Conventions
- Use **Form Requests** for all validation — never validate in controllers
- Use **Service classes** for business logic — never in models or controllers
- Use **Repository pattern** for data access
- Use **`ulid()`** for primary keys (distributed-safe)
- Always use **`DB::transaction()`** for multi-step DB operations
- Queue jobs must implement `ShouldQueue` with `$tries` and `$backoff`
- Use **`php artisan make:`** commands — never create files manually
